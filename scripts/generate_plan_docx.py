...
def main():
    ...
    add_para(doc,
        "v4 的目标：在「诚实的兜底」原则下，让范围外的问题给出有用回答；"
        "同时严格守住校园事务不准 LLM 发言的安全边界。"
    )
    ...
    add_para(doc, "缺点：天气这种实时问题 LLM 会编；校园事务被拒答后再让 LLM 兜底 = 给幻觉开口子；FAQ 模板化与 LLM 自由发挥风格不一致。", bold=True)
    ...
    add_table(doc, ["阶段", "触发条件", "结果"], flow_table_rows,
              col_widths=[3, 5, 6])
    ...
    add_table(doc, ["文件", "动作", "行数变化"], file_rows,
              col_widths=[6, 3, 5])
    ...
    add_table(doc, ["配置项", "默认值", "说明"], tongyi_rows,
              col_widths=[5, 4, 7])
    ...
    add_table(doc, ["配置项", "默认值", "说明"], hf_rows,
              col_widths=[5, 4, 7])
    ...
    add_table(doc, ["风险", "缓解手段"], safety_rows,
              col_widths=[4, 11])

    # ============ 七、系统提示词 ============
    add_h(doc, "七、DeepSeek 系统提示词（防幻觉第一道防线）", 1)
    add_para(doc,
        "下面这段话决定了 LLM 会不会编校务。写得越宽，LLM 越容易跑偏。"
    )
    add_code(doc,
    ...
    add_table(doc, ["测试组", "用例数", "校验点"], test_rows,
              col_widths=[4, 3, 8])
    ...
    add_table(doc, ["扩展项", "思路", "接入位置"], ext_rows,
              col_widths=[3, 6, 5])
