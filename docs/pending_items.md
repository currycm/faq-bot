...
> **§0 的 git 快照已于 2026-09-19 更新为当前状态**（原文停在上表 09-14 那一刻）。

---

## 0. 当前 git 状态（先对齐事实）

> **更新于 2026-09-19** ｜ 语料 v1.16.0（87 意图 / 675 问法）｜ 测试集 242 条

| 项 | 状态 |
|---|---|
| 工作树 | ✅ 干净（`git status --porcelain` 为空） |
| 语料 | v1.16.0，87 意图 / 675 问法；测试集 242 条（可答 239 + 应拒答 3） |
| 测试 | 184 passed（拉起 API 服务后 0 skipped）｜覆盖率 81.1%｜召回@1 99.6% |
| CI | 三道门禁全绿：`ruff check` + `ruff format --check`（**ruff 已锁 0.16.8**）、`--cov-fail-under=80`、`--min-recall 0.98` |
| **未 push** | 🔴 **远端落后于本地，待用户本机 `git push`**（沙箱无 GitHub 凭据） |

<details>
<summary>2026-09-14 的旧快照（留档，已过时）</summary>
...
| ~~D4~~ ✅ | ~~开放集评测~~ **2026-09-15 结论：做不了，已用留出集替代**。核实发现 `logs/unmatched.jsonl` 不能当开放集：5849 行去重仅 210 条、**132 条直接来自 tests/test_set.json**（其余是压测循环），用它就是循环论证；**项目没有真实用户数据**。替代方案见 `docs/holdout_eval_report.md`：每意图留 K 条建索引 —— 8 条→3 条时召回@1 **96.7%→76.5%**，验证了「补问法 > 换算法」这条此前未经验证的核心论断 |
