...
- **128 个单元 + 集成测试**，接入 **GitHub Actions** 做提交自动校验
...
> BGE + 精排（Cross-Encoder）管线已写通（`src/config.py` 设 `ENABLE_RERANK=True`），但需联网下载约 130MB 的 Cross-Encoder 模型；本机无外网故未实测。在 BGE 已 100% 命中的封闭集上精排无提升空间，其真实价值在易混淆/长尾意图，需用更大评测集验证。复现命令：`python scripts/benchmark_retrieval.py`。
...
| v7.1 | 前端去 AI 味：去侧边栏 / 高频问题 chip / 调试信息默认隐藏 | ✅ 当前 |
...
- **限流在多 worker 下不准**：令牌桶是进程内 in-memory，`-w 2` 时实际额度 ≈ 2 倍配置值
  （实测 80 并发放行 52 次 ≈ 2×30）。生产环境要精确限流得换 Redis / API gateway。
- **天气只接了实况接口**：问「明天会下雨吗」返回的是当前天气，尚未接预报 API。
...
| 侧边栏 | 左侧「运行状态 / 反馈统计」两面板常驻 | 整块删除；反馈统计挪到页面底部，仅 `?debug=1` 显示 |
| 高频问题 | 只在首屏出现一次，问完即被对话顶走 | 常驻输入栏上方，7 个可点 chip |

### 调试模式

排查问题时在 URL 后加 `?debug=1`，才显示 `trace_id`、相似度、候选题列表、向量化方式、API 地址。
**默认一律不显示**——`tests/test_ui.py` 里有断言钉死这点，防止调试信息漏回界面。
